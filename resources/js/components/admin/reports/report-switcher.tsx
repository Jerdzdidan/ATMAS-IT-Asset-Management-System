...
/**
 * Jumps between reports without a trip back to the catalogue, which is the difference between
 * comparing two reports in a meeting and losing the thread.
 */
    ...
    return (
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <Button variant="outline" className="justify-between gap-2 sm:min-w-60">
                    <span className="truncate">{current?.title ?? 'Select report'}</span>
                    <ChevronsUpDown className="size-4 opacity-60" />
                </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="start" className="max-h-96 w-76 overflow-y-auto">
