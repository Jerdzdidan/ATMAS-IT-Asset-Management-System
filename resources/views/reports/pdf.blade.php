{{--
    Printable version of any report produced by App\Services\ReportBuilder.

    Styled inline because DomPDF resolves neither the Vite bundle nor external stylesheets.
--}}
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        body { font-family: 'DejaVu Sans', sans-serif; font-size: 8.5px; color: #24242a; margin: 0; }
        ...
        .report-note { color: #6b6b75; font-size: 8.5px; }
        .meta { text-align: right; color: #6b6b75; font-size: 8px; line-height: 1.5; }
        .summary { width: 100%; border-collapse: separate; border-spacing: 5px 0; margin-bottom: 12px; }
        .summary td { background: #f4f6fb; border: 1px solid #dde3f0; border-radius: 3px; padding: 6px 8px; width: 20%; }
        .summary .label { color: #6b6b75; font-size: 7.5px; text-transform: uppercase; letter-spacing: 0.5px; }
        ...
        .signoff td { width: 33%; padding-top: 26px; font-size: 8px; color: #6b6b75; }
...
<body>
    <table class="masthead" width="100%">
        <tr>
            ...
            <td width="46" style="vertical-align: top;"><img src="{{ $logo }}" alt="" width="38" height="38"></td>
            <td>
    ...
    @if (! empty($report['summary']))
        <table class="summary">
            <tr>
                @foreach ($report['summary'] as $figure)
                    <td>
                        <div class="label">{{ $figure['label'] }}</div>
                        <div class="value">{{ $figure['value'] }}</div>
                    </td>
                @endforeach
            </tr>
        </table>
